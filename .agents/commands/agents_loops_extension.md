...
> Add this section to the top of your AGENTS.md (or use as standalone if no AGENTS.md exists yet).
> This becomes active after running `.agents/commands/PRIME_FEEDBACK.md` once.
...
Use the exact commands from `.agents/PROJECT_LOOP.md`.
If that file doesn't exist: run PRIME_FEEDBACK first.

**2. Read the logs**
Read every relevant log file from `.agents/PROJECT_LOOP.md`.
...
Use the test command from `.agents/PROJECT_LOOP.md` or the project's standard test command.
...
If the project has multiple services (as defined in `.agents/PROJECT_LOOP.md`):
...
## When `.agents/PROJECT_LOOP.md` Does Not Exist
...
2. Execute `.agents/commands/PRIME_FEEDBACK.md`
